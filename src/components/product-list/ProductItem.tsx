...
const ProductItem: FC<IProps> = ({ data }) => {
  ...
  return (
    <Link href={`/product/${id}`} className='flex  w-full rounded-lg overflow-hidden border-1'>
      <div className='w-1/3'>
        <Image src={images[0]?.url ?? '/image_placeholder.svg'} alt={title} width={300} height={300} className='w-full'/>
      </div>
